...
<template>
    <Skeleton class="h-80 w-full rounded-xl max-w-xs mx-auto mb-8" v-if="store.loadSkeleton" />
        <Card class="max-w-xs mx-auto mb-8 bg-neutral-900 border-gray-600 px-4 py-4" v-else-if="store.loadSkeleton === false">
            <CardHeader class="p-0 mb-4">
            <CardTitle class="text-white text-xl">Currency Converter</CardTitle>
            ...
            <CardContent class="p-0">
                <div class="grid items-center w-full gap-4 text-white grid-cols-2 mb-4">
                    <div class="flex flex-col space-y-1.5">
                        ...
                        <Input id="ngn" type="number" v-model="ngnInitialAmount" class="bg-neutral-900 border-gray-500"/>
                    </div>
                    <div class="flex flex-col space-y-1.5">
                        <Label for="for" class="text-white">{{ store.selectedCountry }}</Label>
                        <Input id="for" type="number" v-model="localizedAmount" class="bg-neutral-900 border-gray-500"/>
                    </div>
                </div>
                ...
                <Separator class="bg-gray-600 w-full mb-4" />
            </CardContent> 
            <CardFooter class="p-4 text-stone-50 border border-gray-600 rounded-lg">
                    <div class="flex items-center space-x-4 rounded-md w-full">
